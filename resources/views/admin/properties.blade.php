<x-layouts.app>
    ...
    <div class="rounded-lg bg-white p-6 shadow dark:bg-gray-800">
        ...
        <form method="POST"
            action="{{ $editProperty ? route('properties.update', $editProperty) : route('properties.store') }}">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input class="w-full rounded border px-3 py-2 dark:bg-gray-700" name="name" type="text"
                        value="{{ old('name', $editProperty->name ?? '') }}" required>
                    @error('name')
                ...
                <div>
                    ...
                    <input class="w-full rounded border px-3 py-2 dark:bg-gray-700" name="address" type="text"
                        value="{{ old('address', $editProperty->address ?? '') }}" required>
                    @error('address')
                        <p class="mt-1 text-xs font-semibold text-red-500">{{ $message }}</p>
                    @enderror
                </div>
    ...
    <div
        ...
        <div class="border-b border-gray-100 px-6 py-4 dark:border-gray-700">
            <h2 class="text-lg font-bold text-gray-800 dark:text-white">Properties</h2>
        </div>
