<x-layouts.app>
        ...
        <form method="POST" action="{{ $editUnit ? route('units.update', $editUnit) : route('units.store') }}">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                <div class="flex items-center space-x-2">
                    <input name="has_electricity" type="checkbox" value="1" @checked(old('has_electricity', $editUnit->has_electricity ?? false))>
                    <label class="text-sm">Has Electricity (only for shops)</label>
                </div>
                ...
                <div class="mt-4">
                    ...
                    @if ($editUnit)
                        <a class="ml-3 text-sm text-gray-500" href="{{ route('units.index') }}">
                            Cancel
                        </a>
                    @endif
                </div>
    ...
    <div
        ...
        <div class="border-b border-gray-100 px-6 py-4 dark:border-gray-700">
            <h2 class="text-lg font-bold text-gray-800 dark:text-white">Units</h2>
        </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr
                        ...
                        <th class="px-6 py-3 text-left font-semibold">Unit No</th>
                        <th class="px-6 py-3 text-left font-semibold">Type</th>
                        <th class="px-6 py-3 text-left font-semibold">Monthly Rent</th>
                        <th class="px-6 py-3 text-left font-semibold">Electricity</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    @forelse($units as $unit)
                        <tr class="transition-colors duration-200 hover:bg-indigo-50/50 dark:hover:bg-gray-700/50">
                            <td class="px-6 py-4 font-mono text-indigo-500 dark:text-indigo-400">{{ $unit->id }}
                            </td>
                            <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">{{ $unit->property->name }}
                            </td>
                            <td class="px-6 py-4 text-gray-600 dark:text-gray-400">{{ $unit->unit_no }}</td>
                            <td class="px-6 py-4 capitalize text-gray-600 dark:text-gray-400">{{ $unit->type }}</td>
                            <td class="px-6 py-4 text-gray-600 dark:text-gray-400">
                                {{ number_format($unit->monthly_rent, 2) }}</td>
                            <td class="px-6 py-4 text-gray-600 dark:text-gray-400">
                                {{ $unit->has_electricity ? 'Yes' : 'No' }}</td>
                    ...
                    @empty
                        <tr>
                            <td class="py-12 text-center text-gray-500 dark:text-gray-400" colspan="7">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p>No units found in the database.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
